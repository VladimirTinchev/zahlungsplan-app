...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 8, "Die Betraege sind brutto.")
        self.ln(8)
...
def generate_pdf(df, mietername, adresse, vertragsnummer):
    ...
    konto_infos = f'''
Zahlungsempfaenger & Kontoverbindungen:
...
2. Werbebeitraege - Kontoinhaber: HBB Centermanagement GmbH & Co. KG
...
miete = st.number_input("Monatlich: Miete + Nebenkosten", min_value=0.0, step=0.01)
werbung = st.number_input("Halbjaehrlich: Werbebeitrag", min_value=0.0, step=0.01)
gastro = st.number_input("Monatlich: Gastro (Fettabluft)", min_value=0.0, step=0.01)
